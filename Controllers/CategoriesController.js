...
class CategoriesController extends BaseController {
  constructor(model, contractModel, creatorModel) {
    super(model);
...
ORDER BY  month, categories.name;`
      ...
      //       `SELECT
      // creators.id, EXTRACT(MONTH FROM contracts.start_date) as month, categories.name, COUNT(contracts.id)
      // from contracts
      // LEFT JOIN contract_categories ON contracts.id = contract_categories.contract_id
      // LEFT JOIN categories ON contract_categories.category_id = categories.id
      // LEFT JOIN creators ON creators.id = contracts.creator_id
      // WHERE EXTRACT(YEAR FROM contracts.start_date) = ${fiscalYear}
      // AND creators.id = 1
      // GROUP BY creators.id, EXTRACT(MONTH FROM contracts.start_date), categories.name
      // ORDER BY  month, categories.name;`;
  ...
  // creators
  async getCreatorMthlyCategoriesData(req, res) {
